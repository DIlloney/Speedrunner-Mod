...
public class ElderGuardianEntityMixin extends GuardianEntity {
    ...
    @Deprecated
    protected ElderGuardianEntityMixin(EntityType<? extends GuardianEntity> entityType, World world) {
        super(entityType, world);
    }

    /**
     * Changes the Elder Guardians health.
     * @author Dilloney
     * @reason
     */
    @Overwrite
    public static DefaultAttributeContainer.Builder createElderGuardianAttributes() {
        if (OPTIONS.getModDifficulty() == 1) {
            ...
            return OPTIONS.getModDifficulty() == 4 ? GuardianEntity.createGuardianAttributes().add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.30000001192092896D).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 8.0D).add(EntityAttributes.GENERIC_MAX_HEALTH, 36.0D) : GuardianEntity.createGuardianAttributes().add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.30000001192092896D).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 4.0D).add(EntityAttributes.GENERIC_MAX_HEALTH, 25.0D);
        }
    }

    /**
     * Changes what the mob does every tick.
     * @author Dilloney
     * @reason
     */
    @Overwrite
    public void mobTick() {
        super.mobTick();
        if ((this.age + this.getId()) % 600 == 0 && OPTIONS.doomMode) {
        ...
        } else if ((this.age + this.getId()) % 6000 == 0) {
            StatusEffect statusEffect = StatusEffects.MINING_FATIGUE;
            List<ServerPlayerEntity> list = ((ServerWorld)this.world).getPlayers((serverPlayerEntityx) -> {
