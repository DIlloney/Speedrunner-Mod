...
class ModOptionsScreen {
    ...
    protected static ConfigBuilder openScreen() {
        ...
        speedrunnermodoptions.addEntry(entryBuilder.startBooleanToggle(new TranslatableText("options.doom_mode"), options.doomMode).setDefaultValue(options.doomMode).setTooltip(new TranslatableText("options.doom_mode.tooltip")).setSaveConsumer(newValue -> options.doomMode = newValue).build());
        return builder;
    }
}