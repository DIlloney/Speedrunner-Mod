...
public class ModOptionsScreen extends GameOptionsScreen {
    ...
    protected void init() {
        ...
        height += 24;
        this.addDrawableChild(new ButtonWidget(leftSide, height, 150, 20, new TranslatableText("speedrunnermod.menu.options.world_generation"), (buttonWidget) -> {
            this.client.setScreen(new WorldGenerationOptionsScreen(this.parent, MinecraftClient.getInstance().options));
        }));
        this.addDrawableChild(new ButtonWidget(rightSide, height, 150, 20, new TranslatableText("speedrunnermod.menu.options.client"), (buttonWidget) -> {
            this.client.setScreen(new ClientOptionsScreen(this.parent, MinecraftClient.getInstance().options));
        }));

        height += 24;
        this.addDrawableChild(new ButtonWidget(leftSide, height, 150, 20, new TranslatableText("speedrunnermod.menu.options.miscellaneous"), (buttonWidget) -> {
            this.client.setScreen(new MiscellaneousOptionsScreen(this.parent, MinecraftClient.getInstance().options));
        }));
        this.addDrawableChild(new ButtonWidget(rightSide, height, 150, 20, new TranslatableText("speedrunnermod.menu.options.all"), (buttonWidget) -> {
            this.client.setScreen(new AllOptionsScreen(this.parent, MinecraftClient.getInstance().options));
        }));

        height += 24;
        this.addDrawableChild(new ButtonWidget(leftSide, height, 150, 20, new TranslatableText("speedrunnermod.menu.options.reset"), (buttonWidget) -> {
