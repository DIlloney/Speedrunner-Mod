...
/**
 * Used to create {@link net.dillon8775.speedrunnermod.SpeedrunnerMod} features screens.
 */
...
public abstract class AbstractFeatureScreen extends GameOptionsScreen {
    ...
    protected void init() {
        ...
        if (this.getScreenType() == ScreenType.STARTER) {
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, ModTexts.NEXT, (buttonWidget) -> this.client.setScreen(getNextScreen())));

            height += 24;
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, ScreenTexts.DONE, (button) -> this.close()));
        } else if (this.getScreenType() == ScreenType.NORMAL) {
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, ModTexts.NEXT, (buttonWidget) -> this.client.setScreen(getNextScreen())));

            height += 24;
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, ModTexts.PREVIOUS, (buttonWidget) -> this.client.setScreen(getPreviousScreen())));

            height += 24;
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, ScreenTexts.DONE, (button) -> this.close()));
        } else if (this.getScreenType() == ScreenType.FINAL) {
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, this.category1Text, (buttonWidget) -> this.client.setScreen(category1Screen)));

            height += 24;
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, this.category2Text, (buttonWidget) -> this.client.setScreen(category2Screen)));

            height += 24;
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, this.category3Text, (buttonWidget) -> this.client.setScreen(category3Screen)));

            if (hasFourthCategory) {
                height += 24;
                this.addDrawableChild(new ButtonWidget(width, height, 150, 20, this.category4Text, (buttonWidget) -> this.client.setScreen(category4Screen)));
            }
            ...
            height += 24;
            this.addDrawableChild(new ButtonWidget(width, height, 150, 20, ScreenTexts.DONE, (buttonWidget) -> this.close()));
        }
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredText(matrices, this.textRenderer, new LiteralText("§lPage:§r " + this.pageNumber + "/" + this.getMaxPages()), farRightSide, height, 16777215);
    ...
    private TranslatableText getFirstLineText() {
        return new TranslatableText("speedrunnermod.features" + this.linesCategory() + this.linesKey() + ".line1");
    }

    private TranslatableText getSecondLineText() {
        return new TranslatableText("speedrunnermod.features" + this.linesCategory() + this.linesKey() + ".line2");
    }

    private TranslatableText getThirdLineText() {
        return new TranslatableText("speedrunnermod.features" + this.linesCategory() + this.linesKey() + ".line3");
    }

    private TranslatableText getFourthLineText() {
        return new TranslatableText("speedrunnermod.features" + this.linesCategory() + this.linesKey() + ".line4");
    }

    private TranslatableText getFifthLineText() {
        return new TranslatableText("speedrunnermod.features" + this.linesCategory() + linesKey() + ".line5");
    }
