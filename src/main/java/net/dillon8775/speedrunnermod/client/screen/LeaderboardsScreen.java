...
public class LeaderboardsScreen extends AbstractModScreen {
    ...
    public LeaderboardsScreen(Screen parent, GameOptions options) {
        super(parent, options, new TranslatableText("speedrunnermod.title.leaderboards"));
    }
    ...
    protected void init() {
        ...
        ButtonWidget submitSpeedrun = this.addDrawableChild(new ButtonWidget(this.getButtonsLeftSide(), height, 150, 20, new TranslatableText("speedrunnermod.menu.leaderboards.submit").formatted(getSubmitSpeedrunColor()), (button) -> {
            this.openLink(ModLinks.LEADERBOARDS_SUBMISSION_LINK, true);
        }, (button, matrices, x, y) -> {
            if (!options().main.leaderboardsMode) {
                this.renderOrderedTooltip(matrices, this.client.textRenderer.wrapLines(new TranslatableText("speedrunnermod.leaderboards_mode_disabled.tooltip"), 200), x, y);
            } else if (!Leaderboards.isEligibleForLeaderboardRuns()) {
                this.renderOrderedTooltip(matrices, this.client.textRenderer.wrapLines(new TranslatableText("speedrunnermod.cannot_submit_speedrun.tooltip"), 200), x, y);
            } else {
                this.renderOrderedTooltip(matrices, this.client.textRenderer.wrapLines(new TranslatableText("speedrunnermod.submit_speedrun.tooltip"), 200), x, y);
            }
        }));
