...
public abstract class AbstractModScreen extends GameOptionsScreen {
    ...
    protected void init() {
        if (isOptionsScreen()) {
            ...
            }, (button, matrices, x, y) -> this.renderOrderedTooltip(matrices, this.client.textRenderer.wrapLines(Text.translatable("speedrunnermod.help"), 200), x, y)));
            this.addDrawableChild(new ButtonWidget(this.getButtonsRightSide() + 128, this.getDoneButtonsHeight(), 20, 20, Text.literal("D."), (button) -> {
                this.close();
                Util.getOperatingSystem().open(this.configDirectory);
            }, (button, matrices, x, y) -> this.renderOrderedTooltip(matrices, this.client.textRenderer.wrapLines(Text.translatable("speedrunnermod.directory"), 200), x, y)));
        } else {
