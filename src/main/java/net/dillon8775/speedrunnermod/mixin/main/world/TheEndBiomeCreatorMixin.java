...
public class TheEndBiomeCreatorMixin {
    ...
    private static Biome createEndBiome(GenerationSettings.Builder builder) {
        ...
        return DOOM_MODE ?
                new Biome.Builder().precipitation(Biome.Precipitation.NONE).category(Biome.Category.THEEND).temperature(0.5f).downfall(0.5f).effects(new BiomeEffects.Builder().waterColor(4159204).waterFogColor(329011).fogColor(0xA080A0).skyColor(0).particleConfig(new BiomeParticleConfig(ParticleTypes.CRIMSON_SPORE, 0.030F)).moodSound(BiomeMoodSound.CAVE).build()).spawnSettings(builder2.build()).generationSettings(builder.build()).build() :
