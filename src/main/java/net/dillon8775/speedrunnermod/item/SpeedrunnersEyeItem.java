    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (!world.isClient) {
            ...
            if (player.isSneaking() && serverWorld.getRegistryKey().equals(World.OVERWORLD)) {
                ...
                player.sendMessage(new TranslatableText("item.speedrunnermod.speedrunners_eye.looking_for", structureType).formatted(CLModOptions.isActionbar() ? Formatting.AQUA : Formatting.WHITE), CLModOptions.isActionbar());
        ...
        if (!player.isSneaking()) {
            if (!world.isClient) {
                ...
                serverWorld.getRegistryKey(); {
                    ...
                    player.sendMessage(new TranslatableText("item.speedrunnermod.speedrunners_eye.blocks_away", structureType, structureDistance).formatted(CLModOptions.isActionbar() ? Formatting.AQUA : Formatting.WHITE), CLModOptions.isActionbar());
                    return TypedActionResult.success(itemStack);
                }
            }
        }
