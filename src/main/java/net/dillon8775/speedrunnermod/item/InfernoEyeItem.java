...
public class InfernoEyeItem extends Item {
        ...
        if (!world.isClient) {
            if (world.getRegistryKey() == World.NETHER) {
                if (player.isSneaking()) {
                    ...
                    player.sendMessage(new TranslatableText("item.speedrunnermod.eye_of_inferno.looking_for", structureType).formatted(ItemUtil.toFormatting(Formatting.RED, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                } else {
                    ItemUtil.findStructureAndShoot(world, player, itemStack, type);
                    player.sendMessage(new TranslatableText("item.speedrunnermod.eye_of_inferno.located", structureType).formatted(ItemUtil.toFormatting(Formatting.RED, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                ...
                player.sendMessage(new TranslatableText("item.speedrunnermod.eye_of_inferno.wrong_dimension").formatted(ItemUtil.toFormatting(Formatting.RED, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
            }
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(new TranslatableText("item.speedrunnermod.eye_of_inferno.looking_for.tooltip", structureType).formatted(Formatting.BOLD));
    }
}