...
public class PiglinAwakenerItem extends Item {
                        ...
                        if (isSafe) {
                            if (hasGold) {
                                if (confirm) {
                                    ...
                                    player.sendMessage(new TranslatableText("item.speedrunnermod.piglin_awakener.got_piglins").formatted(ItemUtil.toFormatting(Formatting.GOLD, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                                    player.sendMessage(new TranslatableText("item.speedrunnermod.piglin_awakener.confirm"), false);
                                    for (PiglinEntity piglin : piglins) {
                                ...
                                player.sendMessage(new TranslatableText("item.speedrunnermod.piglin_awakener.no_gold_ingot").formatted(ItemUtil.toFormatting(Formatting.RED, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                            }
                        } else {
                            ...
                            player.sendMessage(new TranslatableText("item.speedrunnermod.piglin_awakener.unsafe").formatted(ItemUtil.toFormatting(Formatting.RED, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                        }
                    } else {
                        player.sendMessage(new TranslatableText("item.speedrunnermod.piglin_awakener.couldnt_find_piglins").formatted(ItemUtil.toFormatting(Formatting.RED, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                    }
                } else {
                    player.sendMessage(new TranslatableText("item.speedrunnermod.piglin_awakener.wrong_dimension").formatted(ItemUtil.toFormatting(Formatting.RED, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                }
            } else {
                player.sendMessage(new TranslatableText("item.speedrunnermod.item_disabled"), false);
                info("Player " + player.getName().asString() + " (" + player.getUuidAsString() + ") tried to use Piglin Awakener, but is disabled!");
            }
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (options().client.itemTooltips) {
            tooltip.add(new TranslatableText("item.speedrunnermod.piglin_awakener.tooltip"));
        }
    }
}