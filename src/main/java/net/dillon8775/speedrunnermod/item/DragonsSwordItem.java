...
public class DragonsSwordItem extends SwordItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (options().advanced.disableDragonsSword) {
            info("Entity " + attacker.getName().asString() + " (" + attacker.getUuidAsString() + ") tried to use Dragon's Sword, but is disabled!");
            ...
            if (!DOOM_MODE) {
                ...
                if (attacker instanceof PlayerEntity) {
                    ((PlayerEntity)attacker).sendMessage(new TranslatableText("item.speedrunnermod.dragons_sword.failed").formatted(Formatting.LIGHT_PURPLE), false);
                }
            }
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (options().client.itemTooltips) {
            tooltip.add(new TranslatableText("item.speedrunnermod.dragons_sword.tooltip").formatted(DOOM_MODE ? Formatting.STRIKETHROUGH : Formatting.WHITE));
            if (DOOM_MODE) {
                tooltip.add(new TranslatableText("item.speedrunnermod.dragons_sword.doom_mode").formatted(Formatting.RED));
            }
        }
    }
}