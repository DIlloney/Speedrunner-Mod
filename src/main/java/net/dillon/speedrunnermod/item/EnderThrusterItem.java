...
public class EnderThrusterItem extends Item {
            ...
            if (!options().advanced.disableEnderThruster) {
                if (!(world.getRegistryKey() == World.NETHER)) {
                    ...
                    if (y != playerY && !(playerY > y)) {
                        ...
                        player.sendMessage(new TranslatableText("item.speedrunnermod.ender_thruster.couldnt_teleport"), ModOptions.ItemMessages.isActionbar());
                    }
                } else {
                    player.sendMessage(new TranslatableText("item.speedrunnermod.ender_thruster.wrong_dimension").formatted(ItemUtil.toFormatting(Formatting.AQUA, Formatting.WHITE)), ModOptions.ItemMessages.isActionbar());
                }
            } else {
                player.sendMessage(new TranslatableText("item.speedrunnermod.item_disabled"), false);
                info("Player " + player.getName().asString() + " (" + player.getUuidAsString() + ") tried to use Ender Thruster, but is disabled!");
            }
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (options().client.itemTooltips) {
            tooltip.add(new TranslatableText("item.speedrunnermod.ender_thruster.tooltip"));
        }
    }
}