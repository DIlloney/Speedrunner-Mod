        ...
        if (!world.isClient) {
            if (options().stateOfTheArtItems.isDragonsPearlEnabled()) {
                if (world.getRegistryKey() == World.END) {
                    List<EnderDragonEntity> dragons = world.getEntitiesByClass(EnderDragonEntity.class, player.getBoundingBox().expand(options().advanced.dragonsPearlDragonDistanceXYZ.getAtIndex(0), options().advanced.dragonsPearlDragonDistanceXYZ.getAtIndex(1), options().advanced.dragonsPearlDragonDistanceXYZ.getAtIndex(2)), entity -> true);
