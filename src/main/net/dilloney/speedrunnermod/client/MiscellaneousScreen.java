...
class MiscellaneousScreen extends Screen {
    ...
    protected void init() {
        ...
        ButtonWidget reset_structure_settings = this.addButton(new ButtonWidget(this.width / 2 - 155, 40, 150, 20, new TranslatableText("speedrunnermod.reset_structure_settings"), (buttonWidget) -> {
            this.client.openScreen(new ResetStructureSettingsConfirmScreen(this));
        }, (buttonWidget, matrixStack, i, j) -> {
            this.renderTooltip(matrixStack, new TranslatableText("speedrunnermod.reset_structure_settings.tooltip"), i, j);
        }));
        reset_structure_settings.active = SpeedrunnerMod.OPTIONS.makeStructuresMoreCommon;
        ButtonWidget old_structures_settings = this.addButton(new ButtonWidget(this.width / 2 + 5, 40, 150, 20, new TranslatableText("speedrunnermod.old_structure_settings"), (buttonWidget) -> {
            this.client.openScreen(new UseOldStructureSettingsConfirmScreen(this));
        }, (buttonWidget, matrixStack, i, j) -> {
            this.renderTooltip(matrixStack, new TranslatableText("speedrunnermod.old_structure_settings.tooltip"), i, j);
        }));
        old_structures_settings.active = SpeedrunnerMod.OPTIONS.makeStructuresMoreCommon && !ModHelper.getOldStructureSettings();
        ButtonWidget make_world_structures = this.addButton(new ButtonWidget(this.width / 2 - 155, 65, 150, 20, new TranslatableText("speedrunnermod.make_world_structures"), (buttonWidget) -> {
